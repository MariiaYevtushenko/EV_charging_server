...
- **SQL (функції, 30 днів):** [`SQL_scripts/functions/Global_admin_analytics.sql`](SQL_scripts/functions/Global_admin_analytics.sql)  
  Сесії (кількість, середня тривалість, середній kWh, прибуток, середній чек), прибуток **по станціях і по портах**, **пікові години** (день тижня × година), **денна динаміка** прибутку/kWh, **проксі день/ніч** тарифу (за годиною старту сесії), **гарячі міста** (навантаження), метрики **зв’язку бронювань і сесій** (конверсія).
